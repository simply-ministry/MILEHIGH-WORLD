...
## 2025-11-24 - [Terminal Input and Typewriter Skip Race Conditions]
**Learning:** In Unity, `Input.GetKeyDown(KeyCode.Return)` remains true for the entire frame. If a command is submitted via Return, a typewriter reveal starting in that same frame will instantly skip if it shares the same skip trigger. Using `Time.frameCount` to track when a command was submitted allows the typewriter to ignore skip inputs until the next frame.
**Action:** Use `Time.frameCount` validation to decouple command submission from immediate typewriter skipping.

## 2025-11-24 - [Accessible Speaker Contrast]
**Learning:** Standard "Void Purple" (0.6, 0.1, 0.9) fails WCAG AA contrast guidelines (4.5:1) on dark backgrounds. Shifting the hue slightly toward lavender (0.7, 0.45, 1.0) maintains the character's aesthetic while ensuring text remains readable for all users.
**Action:** Always verify speaker colors against WCAG AA standards and adjust luminance/saturation for accessibility.
