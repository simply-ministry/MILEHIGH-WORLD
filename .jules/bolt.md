## 2026-06-25 - Unity Singleton Optimization
**Learning:** In Unity 2021.3+, `Object.FindObjectOfType<T>()` is discouraged for singleton patterns as it performs an expensive internal sort of all objects in the scene by Instance ID before returning the first result. Replacing it with `Object.FindAnyObjectByType<T>()` eliminates this O(N log N) sorting overhead.
**Action:** Always use `FindAnyObjectByType<T>()` or `FindFirstObjectByType<T>()` when retrieving singletons to bypass unnecessary engine-side sorting.
