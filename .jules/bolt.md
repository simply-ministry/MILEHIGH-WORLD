...
## 2024-05-26 - Float Keys in Caches cause GC allocations
**Learning:** Caching `WaitForSeconds` in a `Dictionary<float, WaitForSeconds>` leads to cache misses due to floating-point imprecision, causing unintended heap allocations and GC spikes during Unity coroutines.
**Action:** Always use integer keys (e.g., `Mathf.RoundToInt(time * 1000f)`) when caching objects by a duration to ensure deterministic lookup and avoid redundant allocations.
