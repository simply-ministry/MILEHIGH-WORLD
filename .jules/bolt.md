...
## 2026-03-27 - [Typewriter Yield Allocation Optimization]
**Learning:** In TextMeshPro-based typewriter effects, yielding `new WaitForSeconds` for every revealed character creates significant GC pressure in long cinematic sequences. Caching these yield instructions in a `Dictionary<float, WaitForSeconds>` eliminates these allocations.
**Action:** Always use a static or shared yield instruction cache in coroutines that yield frequently with variable but recurring durations.
