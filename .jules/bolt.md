...
## 2024-05-26 - WaitForSeconds Dictionary Cache Misses
**Learning:** When caching `WaitForSeconds` in a `Dictionary` to prevent GC allocations in Unity Coroutines, using a `float` as the key leads to cache misses due to floating-point precision issues.
**Action:** Use an `int` key representing milliseconds (e.g., via `Mathf.RoundToInt(time * 1000f)`) rather than `float` to ensure consistent cache hits and prevent memory leaks.
