...
## 2024-05-26 - WaitForSeconds Float Dictionary Precision Cache Miss
**Learning:** Caching `WaitForSeconds` in a `Dictionary<float, WaitForSeconds>` leads to cache misses because floating-point precision causes minute differences (e.g., 0.100001f != 0.1f), leading to repeated redundant object allocations and garbage collection pressure in tight typewriter loops.
**Action:** When caching duration-based objects, always convert the `float` duration to an integer key representing milliseconds (e.g., `Mathf.RoundToInt(time * 1000f)`) to ensure reliable dictionary lookups and avoid GC spikes.
