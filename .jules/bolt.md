...
## 2024-04-20 - Dictionary cache misses with float keys
**Learning:** Using `float` as a dictionary key for caching `WaitForSeconds` objects leads to cache misses due to floating-point tolerance variations, resulting in unexpected GC allocations.
**Action:** Use an `int` key instead by calculating milliseconds (e.g., `Mathf.RoundToInt(time * 1000f)`) to ensure reliable cache hits and eliminate GC allocations during Coroutines.
