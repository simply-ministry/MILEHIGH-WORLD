...
## 2026-06-25 - [Unity Singleton Access Optimization]
**Learning:** Legacy 'FindObjectOfType' in Unity 2021.3+ is significantly slower than modern alternatives because it performs an internal sort of all objects of that type by Instance ID. For Singleton patterns where any instance (or the only instance) is sufficient, 'FindAnyObjectByType' provides a raw O(N) search without the sorting overhead.
**Action:** Always replace legacy 'FindObjectOfType' with 'FindAnyObjectByType' (or 'FindFirstObjectByType' if order matters) to eliminate unnecessary sorting in singleton accessors or manager lookups.
