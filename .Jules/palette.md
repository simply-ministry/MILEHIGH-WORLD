...
## 2026-05-10 - [Layout-Safe Typewriter with Completion Cues]
**Learning:** In Unity TextMeshPro, switching from string concatenation to `maxVisibleCharacters` for typewriter effects is essential for layout stability. Pre-appending interactive cues (like '▽') and forcing a mesh update before the reveal ensures the final layout is calculated upfront, preventing jarring text re-wraps.
**Action:** Always use `maxVisibleCharacters` for dialogue reveal and pre-append completion cues to ensure visual stability.
